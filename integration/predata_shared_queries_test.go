...
var _ = Describe("cbcopy integration tests", func() {
	...
	Describe("GetConstraints", func() {
		var (
			...
			domainConstraint         builtin.Constraint
			constraintInSchema       builtin.Constraint
		)

		BeforeEach(func() {
			uniqueConstraint = builtin.Constraint{Oid: 0, Schema: "public", Name: "uniq2", ConType: "u", Def: sql.NullString{String: "UNIQUE (a, b)", Valid: true}, OwningObject: "public.constraints_table", IsDomainConstraint: false, IsPartitionParent: false}
			fkConstraint = builtin.Constraint{Oid: 0, Schema: "public", Name: "fk1", ConType: "f", Def: sql.NullString{String: "FOREIGN KEY (b) REFERENCES public.constraints_table(b)", Valid: true}, OwningObject: "public.constraints_other_table", IsDomainConstraint: false, IsPartitionParent: false}
			pkConstraint = builtin.Constraint{Oid: 0, Schema: "public", Name: "pk1", ConType: "p", Def: sql.NullString{String: "PRIMARY KEY (b)", Valid: true}, OwningObject: "public.constraints_table", IsDomainConstraint: false, IsPartitionParent: false}
			checkConstraint = builtin.Constraint{Oid: 0, Schema: "public", Name: "check1", ConType: "c", Def: sql.NullString{String: "CHECK (a <> 42)", Valid: true}, OwningObject: "public.constraints_table", IsDomainConstraint: false, IsPartitionParent: false}
			...
			domainConstraint = builtin.Constraint{Oid: 0, Schema: "public", Name: "check1", ConType: "c", Def: sql.NullString{String: "CHECK (VALUE <> 42)", Valid: true}, OwningObject: "public.constraint_domain", IsDomainConstraint: true, IsPartitionParent: false}
			constraintInSchema = builtin.Constraint{Oid: 0, Schema: "testschema", Name: "uniq2", ConType: "u", Def: sql.NullString{String: "UNIQUE (a, b)", Valid: true}, OwningObject: "testschema.constraints_table", IsDomainConstraint: false, IsPartitionParent: false}
...
PARTITION BY RANGE (date)
      ...
      END (date '2008-04-01') EXCLUSIVE);`)
			...
			It("returns a constraint array for only the tables included in the backup set", func() {
				...
				constraintsOid := testutils.OidFromObjectName(connectionPool, "public", "constraints_table", builtin.TYPE_RELATION)
				otherOid := testutils.OidFromObjectName(connectionPool, "public", "other_table", builtin.TYPE_RELATION)
				tables := []builtin.Relation{{Oid: constraintsOid, Schema: "public", Name: "constraints_table"}}
				constraints := builtin.GetConstraints(connectionPool, tables...)
				Expect(constraints).To(HaveLen(1))
				structmatcher.ExpectStructsToMatchExcluding(&constraints[0], &uniqueConstraint, "Oid")

				tables = []builtin.Relation{{Oid: otherOid, Schema: "public", Name: "other_table"}}
				constraints = builtin.GetConstraints(connectionPool, tables...)
				Expect(constraints).To(BeEmpty())
			})
			/*			*/
		...
		Context("Multiple constraints 5+", func() {
			It("returns a constraint array for a table with multiple constraints", func() {
				...
				fkConstraint = builtin.Constraint{Oid: 0, Schema: "public", Name: "fk1", ConType: "f", Def: sql.NullString{String: "FOREIGN KEY (a, b) REFERENCES public.constraints_table(a, b)", Valid: true}, OwningObject: "public.constraints_other_table", IsDomainConstraint: false, IsPartitionParent: false}
				if connectionPool.Version.AtLeast("6") {
					fkConstraint.ConIsLocal = true
				}
				structmatcher.ExpectStructsToMatchExcluding(&constraints[1], &fkConstraint, "Oid")

				pkConstraint = builtin.Constraint{Oid: 0, Schema: "public", Name: "pk1", ConType: "p", Def: sql.NullString{String: "PRIMARY KEY (a, b)", Valid: true}, OwningObject: "public.constraints_table", IsDomainConstraint: false, IsPartitionParent: false}
				if connectionPool.Version.AtLeast("6") {
					pkConstraint.ConIsLocal = true
				}
				structmatcher.ExpectStructsToMatchExcluding(&constraints[2], &pkConstraint, "Oid")
		...
		It("returns a constraint array for a table with PRIMARY KEY constraint that includes non-key columns", func() {
			...
			constraints := builtin.GetConstraints(connectionPool)
			expectedConstraint := builtin.Constraint{Oid: 0, Schema: "public", Name: "table_with_constr_pkey", ConType: "p", Def: sql.NullString{String: "PRIMARY KEY (a, b) INCLUDE (c, d)", Valid: true}, ConIsLocal: true, OwningObject: "public.table_with_constr", IsDomainConstraint: false, IsPartitionParent: false}
	...
	Describe("GetAccessMethods", func() {
		It("returns information for user defined access methods", func() {
			...
			accessMethods := builtin.GetAccessMethods(connectionPool)

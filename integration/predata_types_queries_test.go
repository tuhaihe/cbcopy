...
var _ = Describe("cbcopy integration tests", func() {
	Describe("Get[type]Types functions", func() {
		var (
			...
			baseTypeDefault builtin.BaseType
			//baseTypeCustom  builtin.BaseType
			compositeType builtin.CompositeType
			enumType      builtin.EnumType
			enumType2     builtin.EnumType
		)
		BeforeEach(func() {
		...
		It("returns a slice of composite types", func() {
			...
			results := builtin.GetCompositeTypes(connectionPool)
		...
		It("returns a slice of composite types with collations", func() {
			...
			Expect(results).To(HaveLen(1))
			compositeType.Attributes = []builtin.Attribute{
				{Name: "name", Type: "integer"},
				{Name: "name2", Type: "numeric(8,2)"},
		...
		*/
		It("returns a slice for an enum type", func() {
			...
			results := builtin.GetEnumTypes(connectionPool)
		...
		It("does not return types for sequences or views", func() {
			...
			bases := builtin.GetBaseTypes(connectionPool)
			composites := builtin.GetCompositeTypes(connectionPool)
		...
		It("returns a slice for a domain type", func() {
			...
			results := builtin.GetDomainTypes(connectionPool)
	...
	COLLATION = public.some_coll
			...
			results := builtin.GetRangeTypes(connectionPool)

			Expect(len(results)).To(Equal(1))

			expectedRangeType := builtin.RangeType{
				Oid:            0,
				Schema:         "public",
				Name:           "textrange",
				SubType:        "text",
	...
	Describe("GetCollations", func() {
		...
		It("returns a slice of with specific collation", func() {
			...
			collationDef := builtin.Collation{Oid: 0, Schema: "public", Name: "some_coll", Collate: "de_DE", Ctype: "de_DE", Provider: "c", IsDeterministic: "true"}
			structmatcher.ExpectStructsToMatchExcluding(&collationDef, &results[0], "Oid")

...
var _ = Describe("queries_acl tests", func() {
	Describe("GetMetadataForObjectType", func() {
		var params builtin.MetadataQueryParams
		...
		BeforeEach(func() {
			params = builtin.MetadataQueryParams{ObjectType: "RELATION", NameField: "name", OwnerField: "owner", CatalogTable: "table"}
		})
	...
	ORDER BY o.oid`, securityLabelSelectReplace, sharedSecurityLabelJoinReplace))).WillReturnRows(emptyRows)
		...
		It("returns metadata for multiple objects", func() {
			...
			params.ACLField = "acl"
			resultMetadataMap := builtin.GetMetadataForObjectType(connectionPool, params)

			expectedOne := builtin.ObjectMetadata{Privileges: []builtin.ACL{
				{Grantee: "gpadmin", Insert: true},
				{Grantee: "testrole", Insert: true},
			}, Owner: "testrole"}
			expectedTwo := builtin.ObjectMetadata{Privileges: []builtin.ACL{}, Owner: "testrole", Comment: "This is a metadata comment."}
			resultOne := resultMetadataMap[builtin.UniqueID{Oid: 1}]
			resultTwo := resultMetadataMap[builtin.UniqueID{Oid: 2}]
	...
	Describe("GetCommentsForObjectType", func() {
		...
		BeforeEach(func() {
			params = builtin.MetadataQueryParams{NameField: "name", OidField: "oid", CatalogTable: "table"}
		})
	...
	FROM table o JOIN pg_description d ON (d.objoid = oid AND d.classoid = 'table'::regclass AND d.objsubid = 0)`)).WillReturnRows(emptyRows)
			builtin.GetCommentsForObjectType(connectionPool, params)
		})
	...
	FROM table o JOIN pg_shdescription d ON (d.objoid = oid AND d.classoid = 'table'::regclass)`)).WillReturnRows(emptyRows)
		...
		It("returns comments for multiple objects", func() {
			...
			mock.ExpectQuery(`SELECT (.*)`).WillReturnRows(fakeRows)
			resultMetadataMap := builtin.GetCommentsForObjectType(connectionPool, params)

			expectedOne := builtin.ObjectMetadata{Privileges: []builtin.ACL{}, Comment: "This is a metadata comment."}
			expectedTwo := builtin.ObjectMetadata{Privileges: []builtin.ACL{}, Comment: "This is also a metadata comment."}
			resultOne := resultMetadataMap[builtin.UniqueID{Oid: 1}]
			resultTwo := resultMetadataMap[builtin.UniqueID{Oid: 2}]
			Expect(resultMetadataMap).To(HaveLen(2))
			structmatcher.ExpectStructsToMatch(&expectedOne, &resultOne)

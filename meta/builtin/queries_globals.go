...
func GetResourceQueues(connectionPool *dbconn.DBConn) []ResourceQueue {
	...
	gplog.Debug("GetResourceQueues, query is %v", query)
...
func GetResourceGroups[T ResourceGroupBefore7 | ResourceGroupAtLeast7](connectionPool *dbconn.DBConn) []T {
	...
	gplog.Debug("GetResourceGroups， query is %v", query)
	results := make([]T, 0)
	err := connectionPool.Select(&results, query)
...
func GetRoles(connectionPool *dbconn.DBConn) []Role {
	...
	gplog.Debug("GetRoles， query is %v", query)
	roles := make([]Role, 0)
...
func GetRoleGUCs(connectionPool *dbconn.DBConn) map[string][]RoleGUC {
	...
	gplog.Debug("GetRoleGUCs，query is %v", query)
...
func getTimeConstraintsByRole(connectionPool *dbconn.DBConn) map[uint32][]TimeConstraint {
	...
	gplog.Debug("getTimeConstraintsByRole， query is %v", query)
...
// https://github.com/greenplum-db/gpbackup/commit/027c410577aaf46d98339487c2588560c78bbd2c
// https://github.com/greenplum-db/gpbackup/commit/dc30c246993a20ab512b4da4e6665aca098b9a4a
func GetRoleMembers(connectionPool *dbconn.DBConn) []RoleMember {
	...
	gplog.Debug("GetRoleMembers, query is %v", query)
...
func GetTablespaces(connectionPool *dbconn.DBConn) []Tablespace {
	...
	var err error
	if connectionPool.Version.Before("6") {
		gplog.Debug("GetTablespaces, query is %v", before6Query)
		err = connectionPool.Select(&results, before6Query)
	} else {
		gplog.Debug("GetTablespaces, query is %v", atLeast6Query)

...
type Function struct {
	...
	Language          string
	Kind              string `db:"prokind"`     // GPDB 7+ // https://github.com/greenplum-db/gpbackup/commit/191004a6d192c13dee9ca1888a132588e5db4753
	PlannerSupport    string `db:"prosupport"`  // GPDB 7+ // https://github.com/greenplum-db/gpbackup/commit/04c31a8b156d962410b0c4d4ca6ca3709ce1e477
	IsWindow          bool   `db:"proiswindow"` // before 7 // https://github.com/greenplum-db/gpbackup/commit/191004a6d192c13dee9ca1888a132588e5db4753
	ExecLocation      string `db:"proexeclocation"`
	Parallel          string `db:"proparallel"` // GPDB 7+ // https://github.com/greenplum-db/gpbackup/commit/08e1f840398596be92d6d34020aed390352c7553
...
func GetFunctions(connectionPool *dbconn.DBConn) []Function {
	...
	gplog.Debug("GetFunctions, query is %v", query)
...
func GetFunctions4(connectionPool *dbconn.DBConn) []Function {
	...
	SELECT p.oid,
		...
		proretset,
		coalesce(prosrc, '') AS functionbody,
		CASE WHEN probin = '-' THEN '' ELSE coalesce(probin, '') END AS binarypath,
		provolatile,
		proisstrict,
		prosecdef,
	...
	gplog.Debug("GetFunctions4, query is %v", query)
	results := make([]Function, 0)
	err := connectionPool.Select(&results, query)
	gplog.FatalOnError(err)
	return results
...
func GetFunctionArgsAndIdentArgs(connectionPool *dbconn.DBConn) (map[uint32]string, map[uint32]string) {
	...
	gplog.Debug("GetFunctionArgsAndIdentArgs, query is %v", query)
	results := make([]struct {
		Oid  uint32
		Type string
		Name string
...
func GetFunctionReturnTypes(connectionPool *dbconn.DBConn) map[uint32]Function {
	...
	gplog.Debug("GetFunctionReturnTypes, query is %v", query)
	results := make([]Function, 0)
	err := connectionPool.Select(&results, query)
	gplog.FatalOnError(err)
...
type Aggregate struct {
	...
	// https://github.com/greenplum-db/gpbackup/commit/4fdf2d8bf1271c36cdb27aaaa48edb2ff1472f40
...
func GetAggregates(connectionPool *dbconn.DBConn) []Aggregate {
	...
	gplog.Debug("GetAggregates, query is %v", query)
...
// which commit add this FQN() and GetMetadataEntry()
...
func GetFunctionOidToInfoMap(connectionPool *dbconn.DBConn) map[uint32]FunctionInfo {
	...
	if connectionPool.Version.Before("5") {
		...
		gplog.Debug("GetFunctionOidToInfoMap, query is %v", atLeast5Query)
...
func GetCasts(connectionPool *dbconn.DBConn) []Cast {
	...
	ORDER BY 1, 2`, argStr, methodStr,
		SchemaFilterClauseNotFilterPgCatalog("sn"), SchemaFilterClauseNotFilterPgCatalog("tn"),
		SchemaFilterClauseNotFilterPgCatalog("n"), ExtensionFilterClause("c"),
		FIRST_NORMAL_OBJECT_ID)

	query := query1 + "\n UNION \n" + query2

	gplog.Debug("GetCasts, query is %v", query)
	casts := make([]Cast, 0)
...
func GetExtensions(connectionPool *dbconn.DBConn) []Extension {
	...
	gplog.Debug("GetExtensions, query is %v", query)
...
func GetProceduralLanguages(connectionPool *dbconn.DBConn) []ProceduralLanguage {
	...
	gplog.Debug("GetProceduralLanguages, query is %v", query)
	err := connectionPool.Select(&results, query)
	gplog.FatalOnError(err)
...
func GetTransforms(connectionPool *dbconn.DBConn) []Transform {
	...
	gplog.Debug("GetTransforms, query is %v", query)
...
func GetConversions(connectionPool *dbconn.DBConn) []Conversion {
	...
	gplog.Debug("GetConversions, query is %v", query)
...
func (fdw ForeignDataWrapper) GetMetadataEntry() (string, toc.MetadataEntry) {
	return "predata",
		toc.MetadataEntry{
			...
			EndByte:         0,
			//Oid:             fdw.Oid,
			//ClassOID:        PG_FOREIGN_DATA_WRAPPER_OID,
		}
}
...
func GetForeignDataWrappers(connectionPool *dbconn.DBConn) []ForeignDataWrapper {
	...
	gplog.Debug("GetForeignDataWrappers, query is %v", query)
...
func (fs ForeignServer) GetMetadataEntry() (string, toc.MetadataEntry) {
	return "predata",
		toc.MetadataEntry{
			...
			EndByte:         0,
			//Oid:             fs.Oid,
			//ClassOID:        PG_FOREIGN_SERVER_OID,
		}
}
...
func GetForeignServers(connectionPool *dbconn.DBConn) []ForeignServer {
	...
	gplog.Debug("GetForeignServers, query is %v", query)
...
func GetUserMappings(connectionPool *dbconn.DBConn) []UserMapping {
	...
	gplog.Debug("GetUserMappings, query is %v", query)
...
type StatisticExt struct {
	...
	Definition  string // https://github.com/greenplum-db/gpbackup/commit/6e0407650ca6ba5f7db53e381c0bddfe997c394c
}
...
func GetExtendedStatistics(connectionPool *dbconn.DBConn) []StatisticExt {
	...
	gplog.Debug("GetExtendedStatistics, query is %v", query)

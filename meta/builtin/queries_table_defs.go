...
func (t Table) GetMetadataEntry() (string, toc.MetadataEntry) {
	...
	return "predata",
		toc.MetadataEntry{
			...
			StartByte:       0,
			EndByte:         0,
			// Oid:             t.Oid,        // gpbackup latest code does not have this
			// ClassOID:        PG_CLASS_OID, // gpbackup latest code does not have this
		}
}
...
type TableDefinition struct {
	...
	PartitionAlteredSchemas []AlteredPartitionRelation // https://github.com/greenplum-db/gpbackup/commit/82c07b3f18e2e8ba6223b7ba3edbfa8c6d3cd359
	AccessMethodName        string                     // https://github.com/greenplum-db/gpbackup/commit/0c7ec1250b57fbf62cc49d3bcbe6b7d3cbcebb76
	PartitionKeyDef         string                     // https://github.com/greenplum-db/gpbackup/commit/ec770a63d2a0778be6f823012af9347ef2cd31e6
	AttachPartitionInfo     AttachPartitionInfo        // https://github.com/greenplum-db/gpbackup/commit/08fd7c563bdf5510ffd9256632adb56fa43d65ff
	ForceRowSecurity        bool                       // https://github.com/greenplum-db/gpbackup/commit/49df00b9351e2c66fa5004b1ecf26d6c6a35ea87
}
...
func GetPartitionTableMap(connectionPool *dbconn.DBConn) map[uint32]PartitionLevelInfo {
	...
	gplog.Debug("GetPartitionTableMap, query is %v", query)
...
type ColumnDefinition struct {
	...
	AttGenerated          string // https://github.com/greenplum-db/gpbackup/commit/52f1ffe8b971d691498cc11e774173546d3dceb4
}
...
// https://github.com/greenplum-db/gpbackup/commit/637c2f04f2b5a62f2be5f89ab82857bbca09d760
func GetColumnDefinitions(connectionPool *dbconn.DBConn) map[uint32][]ColumnDefinition {
	...
	gplog.Debug("GetColumnDefinitions, query is %v", query)
...
func GetTableType(connectionPool *dbconn.DBConn) map[uint32]string {
	...
	query := `
		SELECT oid, reloftype::pg_catalog.regtype AS value FROM pg_class WHERE reloftype != 0`
	return selectAsOidToStringMap(connectionPool, query)
}
...
func GetPartitionDetails(connectionPool *dbconn.DBConn) (map[uint32]string, map[uint32]string) {
	...
	FROM pg_partition p
		...
		JOIN pg_namespace n ON c.relnamespace = n.oid
	WHERE %s`, relationAndSchemaFilterClause(connectionPool))

	gplog.Debug("GetPartitionDetails, query is %v", query)
	var results []struct {
		Oid        uint32
		Definition string
...
func GetPartitionAlteredSchema(connectionPool *dbconn.DBConn) map[uint32][]AlteredPartitionRelation {
	...
	gplog.Debug("GetPartitionAlteredSchema, query is %v", query)
	var results []struct {
		Oid uint32
...
func GetTableStorage(connectionPool *dbconn.DBConn) (map[uint32]string, map[uint32]string) {
	...
	WHERE %s
		...
		relationAndSchemaFilterClause(connectionPool))

	gplog.Debug("GetTableStorage, query is %v", query)
	var results []struct {
		Oid        uint32
...
func GetUnloggedTables(connectionPool *dbconn.DBConn) map[uint32]bool {
	...
	query := `
		SELECT oid FROM pg_class WHERE relpersistence = 'u'`

	gplog.Debug("GetUnloggedTables, query is %v", query)
	var results []struct {
		Oid uint32
	}
	err := connectionPool.Select(&results, query)
...
func GetForeignTableDefinitions(connectionPool *dbconn.DBConn) map[uint32]ForeignTableDefinition {
	...
	gplog.Debug("GetForeignTableDefinitions, query is %v", query)
...
func GetTableInheritance(connectionPool *dbconn.DBConn, tables []Relation) map[uint32][]string {
	...
	gplog.Debug("GetTableInheritance, query is %v", query)
...
func GetPartitionKeyDefs(connectionPool *dbconn.DBConn) map[uint32]string {
	...
	gplog.Debug("GetPartitionKeyDefs, query is %v", query)
	var results []struct {
		Oid    uint32
		Keydef string
	}
...
func GetAttachPartitionInfo(connectionPool *dbconn.DBConn) map[uint32]AttachPartitionInfo {
	...
	gplog.Debug("GetAttachPartitionInfo, query is %v", query)
...
func GetForceRowSecurity(connectionPool *dbconn.DBConn) map[uint32]bool {
	...
	gplog.Debug("GetForceRowSecurity, query is %v", query)
...
func selectAsOidToStringMap(connectionPool *dbconn.DBConn, query string) map[uint32]string {
	gplog.Debug("selectAsOidToStringMap, query is %v", query)
	var results []struct {
		Oid   uint32
		Value string
	}
